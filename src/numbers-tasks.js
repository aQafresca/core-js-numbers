...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const numberX = x1 - x2;
  const numberY = y1 - y2;
  return Math.sqrt(numberX * numberX + numberY * numberY);
}
...
function getFibonacciNumber(index) {
  ...
  const number = getFibonacciNumber(index - 1) + getFibonacciNumber(index - 2);
  return number;
}
...
function getSumOfDigits(num) {
  ...
  const sum = toStr.reduce((a, b) => Number(a) + Number(b));
  const number = sum < 10 ? sum : getSumOfDigits(num);
  return number;
}
...
function toExponential(number, fractionDigits) {
  const string = number.toExponential(fractionDigits);
  return string;
}
